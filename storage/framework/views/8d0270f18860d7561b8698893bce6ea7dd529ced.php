				...
				<table id="chitiet" class="table table-bordered table-hover">
					...
					<tbody id="body">
						<?php foreach($data as $row): ?>
							<tr>
							<td><input type="hidden" name="txtidOld[]" value="<?php echo $row->idPro; ?>"><?php echo $row->idPro; ?></td>
							<td><?php echo $row->name; ?></td>
							<td><input type="number" name="txtSLOld[]" id="SL<?php echo $row->idPro; ?>" value="<?php echo $row->quantity; ?>" class="<?php echo $row->idPro; ?> SL"></td>
							<td><input type="number" name="txtDGOld[]" id="DG<?php echo $row->idPro; ?>" value="<?php echo $row->priceOne; ?>" class="<?php echo $row->idPro; ?> DG"></td>
							<td><input type="text" disabled  id="TT<?php echo $row->idPro; ?>" value="<?php echo $row->quantity * $row->priceOne; ?>" class="money"></td>
							<td><input type="button" class="btn btn-default btn-remove <?php echo $row->idPro; ?>" value="x"></td>
							</tr>
						<?php endforeach; ?>
					</tbody>
				</table>
		...
		<div class="col-md-5">
			...
			<div class="form-group">
				...
				<div class="input-group">
					<input class="form-control date-picker" id="inputMaybeRec" name="txtMaybeRec" type="text" data-date-format="dd-mm-yyyy" value="<?php echo old('txtMaybeRec',isset($bill['maybe_Rec']) ? $bill['maybe_Rec'] : null); ?>" />
			...
			<div class="form-group">
				...
				<div class="input-group">
					<input class="form-control date-picker" id="inputRec" name="txtRec" type="text" data-date-format="dd-mm-yyyy" value="<?php echo old('txtRec',isset($bill['receive_at']) ? $bill['receive_at'] : null); ?>"/>
					<span class="input-group-addon">
						<i class="fa fa-calendar bigger-110"></i>
					</span>
				</div>
...
<script type="text/javascript">
	$( document ).ready(function() {
		//$('#sltSup option[value="'+<?php echo old('sltSup',isset($bill['idSup']) ? $bill['idSup'] : null); ?>+'"]').attr('selected', 'selected');
		$TongTien = 0;
		$(".money").each(function () {
